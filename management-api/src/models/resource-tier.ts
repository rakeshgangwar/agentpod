/**
 * Resource Tier Model
 * Defines available resource tiers (CPU, memory, storage allocations)
 */
...
export interface ResourceTier {
  ...
  cpuCores: number;              // CPU cores (e.g., 1, 2, 4, 8)
  memoryGb: number;              // Memory in GB (e.g., 2, 4, 8, 16)
  storageGb: number;             // Storage in GB (e.g., 20, 30, 50, 100)
...
function rowToTier(row: ResourceTierRow): ResourceTier {
  return {
    id: row.id,
    name: row.name,
 ...
 */
export function getResourceLimitsForTier(tier: ResourceTier): {
  ...
  return {
    ...
    limits_memory_reservation: `${Math.ceil(tier.memoryGb / 2)}g`,
    limits_cpus: `${tier.cpuCores}`,
  };
}
