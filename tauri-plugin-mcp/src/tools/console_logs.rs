...
pub async fn handle_get_console_logs<R: Runtime>(
    ...
    app.once("get-console-logs-response", move |event| {
        info!("[TAURI_MCP] Received get-console-logs-response");
    ...
    match rx.recv_timeout(std::time::Duration::from_secs(5)) {
        ...
        Err(e) => {
            info!("[TAURI_MCP] Timeout waiting for response: {}", e);
            Ok(crate::socket_server::SocketResponse {
                success: false,
                data: None,
                error: Some(format!("Timeout waiting for console logs: {}", e)),
            })
        }
    }
}
